...
import { execute, createProgressEmitter, PROGRESS_THROTTLE_MS, type DispatchContext } from "../src/dispatch.js";
...
function makeResult(agentName: string, text: string, extra: Partial<SingleResult> = {}): SingleResult {
  return {
    agent: agentName,
    agentSource: "user",
    task: "t",
...
describe("live progress through execute()", () => {
  ...
  it("single mode: threads the abort signal to the runner", async () => {
    ...
    await execute({ agent: "a", task: "x" }, baseCtx({ signal: controller.signal }), AGENTS, runner);
  ...
  it("chain mode: snapshots grow step by step; final content is last step output", async () => {
    ...
    const spy = vi.fn();
    const out = await execute(
      { chain: [{ agent: "a", task: "first {previous}" }, { agent: "b", task: "second {previous}" }] },
      baseCtx({ onUpdate: spy, progressIntervalMs: 0 }),
      AGENTS,
      runner,
    );
...
describe("render: partial/running results", () => {
  it("single: running result shows an in-flight marker, not the success check", async () => {
    ...
    const out = renderResult(
      { content: [], details: { mode: "single", agentScope: "user", projectAgentsDir: null, results: [{ ...r, running: true }] } },
      { isPartial: true },
      theme as never,
    );
    ...
    const out = renderResult(
      {
        ...
        details: {
          ...
          results: [{ ...makeResult("a", "done"), running: false }, { ...makeResult("b", "working"), running: true }],
        },
      },
      { isPartial: true },
      theme as never,
