...
describe('renderResult', () => {
    ...
    const r = renderResult(
      {
        ...
        details: details('single', [
          makeResult({ exitCode: 1, stopReason: 'error', errorMessage: 'model 404' }),
        ]),
        isError: true,
      },
      { expanded: false },
  ...
  it('expanded view includes the final assistant text', () => {
    const messages = [
      {
        role: 'assistant' as const,
        content: [{ type: 'text' as const, text: 'FINAL ANSWER TEXT' }],
      },
    ] as unknown as Message[];
    const r = renderResult(
      {
    ...
    const r = renderResult(
      {
        ...
        details: details('parallel', [
          makeResult({ agent: 'a' }),
          makeResult({ agent: 'b', exitCode: 1, stopReason: 'error' }),
        ]),
      },
      { expanded: false },
      theme as never,
