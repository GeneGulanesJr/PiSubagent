...
describe("integration smoke — JSONL through SubprocessRunner", () => {
    ...
    const events = [
      {
        ...
        message: {
          ...
          usage: { input: 100, output: 50, cacheRead: 10, cacheWrite: 0, cost: 0.001, totalTokens: 150 },
        },
      },
      {
        type: "message_end",
        message: {
          ...
          usage: { input: 20, output: 5, cacheRead: 0, cacheWrite: 0, cost: 0.0002, totalTokens: 25 },
  ...
  it("full dispatch path: execute() single mode returns model output as content", async () => {
    ...
    const out = await execute({ agent: "scout", task: "find auth" }, ctx as never, [scoutAgent], runner);
